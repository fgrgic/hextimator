...
| ----------------------------- | --------------------------------------------------------- | ------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `surfaceColor`                | `ColorInput`                                              | Auto-generated from input color with very low chroma          | Surface baseline; override one mode with `light.surfaceColor` / `dark.surfaceColor`                                                                                  |
| `surfaceHueShift`             | `number` (degrees)                                        | `0`                                                           | Rotate surface hue relative to accent. Ignored when `surfaceColor` is set (top-level or for that theme). Per-theme: `light.surfaceHueShift` / `dark.surfaceHueShift` |
| `hueShift`                    | `number` (degrees)                                        | `0`                                                           | Per-variant hue rotation across accent and semantic scales. Per-theme: `light.hueShift` / `dark.hueShift`                                                            |
| `semanticColors`              | `{ positive?: color, negative?: color, warning?: color }` | Auto-generated from seed                                      | Override semantic roles; per theme: `light.semanticColors` / `dark.semanticColors` (unset slots inherit top-level)                                                   |
| `semanticColorRanges`         | `{ positive?: [start, end], ... }`                        | `positive: [120,160]`, `negative: [5,30]`, `warning: [45,70]` | Hue arcs for semantic discovery. Per-theme partial merge: `light.semanticColorRanges` / `dark.semanticColorRanges`                                                   |
...
| `baseLightnessRange`          | `[min, max]` OKLCH **L** (0–1 each)                       | light `[0.4,0.9]`, dark `[0.2, 0.8]`                          | Clamps `baseLightness` and input-derived anchors in both themes; `light` / `dark` can override per theme                                                             |
...
Same optional fields as the top level of `.style()`: `HextimateStyleOptions` **extends** this interface, then adds `light`, `dark`, and `invertDarkModeSurfaceAccent`. Under `light` / `dark`, each field overrides that theme only; omitted keys there inherit from the top-level value.

| Field                 | Type                                 | Default (when not set)                                                                                                    | Description                                                                       |
| --------------------- | ------------------------------------ | ------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------- |
| `baseLightness`       | `number` (0–1)                       | Input OKLCH **L**, clamped using per-theme `baseLightnessRange`, else top-level `baseLightnessRange`, else built-in range |
| `baseLightnessRange`  | `[min, max]` (OKLCH **L**, 0–1 each) | Top-level or built-in per theme                                                                                           | Overrides top-level `baseLightnessRange` for this theme only                      |
| `surfaceColor`        | `ColorInput`                         | Top-level `surfaceColor`                                                                                                  | Surface baseline for this theme only                                              |
| `surfaceHueShift`     | `number` (degrees)                   | Top-level `surfaceHueShift`                                                                                               | Ignored when this theme sets `surfaceColor`                                       |
| `hueShift`            | `number` (degrees)                   | Top-level `hueShift`                                                                                                      | Per-variant hue shift for accent and semantic scales                              |
| `semanticColors`      | `{ positive?, negative?, warning? }` | Top-level `semanticColors`                                                                                                | Unset slots inherit top-level                                                     |
| `semanticColorRanges` | `{ positive?, negative?, warning? }` | Top-level `semanticColorRanges` / defaults                                                                                | Unset slots inherit top-level or library defaults                                 |
