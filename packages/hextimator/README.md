...
// Fork with a different accent color
const warm = base.fork("#ff6677");

// Fork with a different accent color and override an option
const cool = base.fork("#3a86ff", { invertDarkModeBaseAccent: false });

// Fork with only option changes (same accent color)
const muted = base.fork({ baseMaxChroma: 0.01, themeLightness: 0.7 });

// Each fork is a full builder — you can keep chaining
const extended = base
  .fork("#ff6677")
...
const natureTheme = base.format({ as: "css", colors: "hex" });
const warmTheme = warm.format({ as: "css", colors: "hex" });
const coolTheme = cool.format({ as: "tailwind" });
```
