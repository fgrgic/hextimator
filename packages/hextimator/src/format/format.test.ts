...
describe("format() — scss", () => {
  it("prefixes all keys with $", () => {
    const result = format(palette, { format: "scss" }) as Record<string, string>;
    expect(Object.keys(result).every((k) => k.startsWith("$"))).toBe(true);
  });
});
...
describe("format() — tailwind", () => {
  ...
  it("groups all five roles at the top level", () => {
    const result = format(palette, { format: "tailwind" }) as Record<string, unknown>;
    expect(result["base"]).toBeDefined();
    expect(result["accent"]).toBeDefined();
...
describe("format() — options", () => {
  it("respects custom separator", () => {
    const result = format(palette, { separator: "_" }) as Record<string, string>;
  ...
  it("respects colorFormat: rgb", () => {
    const result = format(palette, { colorFormat: "rgb" }) as Record<string, string>;
    expect(Object.values(result).every((v) => v.startsWith("rgb("))).toBe(true);
  });

  it("respects colorFormat: oklch", () => {
    const result = format(palette, { colorFormat: "oklch" }) as Record<string, string>;
    expect(Object.values(result).every((v) => v.startsWith("oklch("))).toBe(true);
  });
});
