...
const BUILDER_METHODS: CompletionEntry[] = [
  {
    ...
    type: 'method',
    detail: '(name, color)',
    info: 'Add a named color role to the palette',
    apply: "addRole('name', '#color')",
  },
  {
    label: 'addVariant',
    type: 'method',
    detail: '(name, placement)',
    info: 'Add a variant (e.g. hover, muted)',
    apply: "addVariant('name', { from: 'strong' })",
  },
  {
    label: 'addToken',
    type: 'method',
    detail: '(name, value)',
    info: 'Add a standalone token',
    apply: "addToken('name', { from: 'accent' })",
  },
  {
    label: 'preset',
    type: 'method',
    detail: '(preset)',
    info: 'Apply a preset configuration',
  ...
  },
  {
    ...
    type: 'method',
    detail: '(adjustments)',
    info: 'Light theme overrides (lightness, maxChroma)',
    apply: 'light({ lightness: 0.7 })',
  },
  {
    label: 'dark',
    type: 'method',
    detail: '(adjustments)',
    info: 'Dark theme overrides (lightness, maxChroma)',
    apply: 'dark({ lightness: 0.6 })',
  },
  {
    label: 'simulate',
    type: 'method',
    detail: '(type, severity?)',
    info: 'Preview CVD simulation (deuteranopia, protanopia, tritanopia, achromatopsia)',
    apply: "simulate('deuteranopia')",
  },
  {
    label: 'adaptFor',
    type: 'method',
    detail: '(type, severity?)',
    info: 'Adapt palette for color vision deficiency',
    apply: "adaptFor('deuteranopia')",
  },
  {
    label: 'fork',
    type: 'method',
  ...
  },
  {
    ...
    type: 'method',
    detail: '(options)',
    info: 'Format the palette for output',
    apply: "format({ as: 'css', colors: 'hex' })",
  },
];
...
const STYLE_OPTIONS: CompletionEntry[] = [
  ...
  {
    label: 'foregroundMaxChroma',
    type: 'property',
    info: 'Max chroma for foreground colors. Default: 0.01',
    detail: 'number',
  },
  {
    label: 'minContrastRatio',
    type: 'property',
    info: "Min WCAG contrast ratio. Default: 'AAA' (7)",
    detail: "'AAA' | 'AA' | number",
  },
  {
    label: 'hueShift',
    type: 'property',
    info: 'Hue shift per variant step (degrees). Default: 0',
    detail: 'number',
  },
  {
  ...
  {
    label: 'semanticColorRanges',
    type: 'property',
    info: 'Hue degree ranges for semantic colors',
    detail: 'object',
  },
];
...
const THEME_ADJUSTMENTS: CompletionEntry[] = [
  {
    label: 'lightness',
    type: 'property',
    info: 'Absolute OKLCH lightness (0–1). Light default: 0.7, dark default: 0.6',
    detail: 'number',
  },
  {
    label: 'maxChroma',
    type: 'property',
    info: 'Max chroma for accent/semantic colors in this theme',
    detail: 'number',
  },
  {
    label: 'minContrastRatio',
    type: 'property',
    info: 'Override global contrast ratio for this theme',
    detail: "'AAA' | 'AA' | number",
  },
  {
    label: 'baseMaxChroma',
  ...
  },
  {
    label: 'foregroundMaxChroma',
    type: 'property',
    info: 'Override global foregroundMaxChroma for this theme',
    detail: 'number',
  },
];
...
const VARIANT_PLACEMENTS: CompletionEntry[] = [
  ...
  },
  {
    label: 'between',
    type: 'property',
    info: 'Place variant between two existing variants',
    detail: '[string, string]',
    apply: "between: ['DEFAULT', 'strong']",
  },
];

const PRESETS: CompletionEntry[] = [
  {
    label: 'presets.shadcn',
    type: 'variable',
...
function matchContext(text: string): CompletionEntry[] | null {
  ...
  if (/\.\s*(?:light|dark)\s*\(\s*\{[^}]*$/.test(text))
    return THEME_ADJUSTMENTS;
...
export function hextimatorCompletions(
  ...
  const fullText = context.state.doc
  ...
  const from =
    context.pos -
    (word.startsWith('presets.')
      ? word.length
      : (word.split('.').pop()?.length ?? 0));
